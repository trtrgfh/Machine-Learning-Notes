...
ML is the field of study that gives computers the ability to learn without explicit programed. <br>
...
- Choices of B
    - large B (e.g. 100, 1000): better result, but slower
    - small B (e.g. 10): worse result, but faster
...
- denote t^{\'} as the time steps for the original text, and t as the time steps for the translation
- $\alpha^{\langle t,t^{\'} \rangle} = \frac{exp(e^{\langle t,t^{\'} \rangle})}{\sum_{t^{\'}=1}^{T_x} e^{\langle t,t^{\'} \rangle}} =$ amount of attention $y^{\langle t \rangle}$ should pay to $a^{\langle t^{\'} \rangle}$
- $e^{\langle t,t^{\'} \rangle}$ can be generated by feeding $s^{\langle t-1 \rangle}$ and $a^{\langle t^{\'} \rangle}$ to an one hidden layer neural network, where $s^{\langle t-1 \rangle}$ is the activations from the previous time step of the translation
